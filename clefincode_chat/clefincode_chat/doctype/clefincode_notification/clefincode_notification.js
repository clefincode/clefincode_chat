...
frappe.notification = {
	...
	get_variable_source_meta: function (frm, row) {
		if (!row || !row.source_type || !row.source_field || !frm.doc.reference_doctype) {
			return null;
		}

		if (row.source_type === "Document Field") {
			const meta_fields = frappe.meta.get_docfields(frm.doc.reference_doctype) || [];
			return meta_fields.find(df => df.fieldname === row.source_field) || null;
		}

		if (row.source_type === "Child Row Field") {
			if (!frm.doc.child_table_field) return null;
			...
			if (!table_df || !table_df.options) return null;

			const child_meta_fields = frappe.meta.get_docfields(table_df.options) || [];
			return child_meta_fields.find(df => df.fieldname === row.source_field) || null;
		}

		return null;
	},

	apply_variable_attachment_state: function (frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (!row) return;

		const df = frappe.notification.get_variable_source_meta(frm, row);
		const fieldtype = df ? (df.fieldtype || "") : "";

		row.field_datatype = fieldtype;
	...
	setup_variable_source_field: function (frm, cdt, cdn) {
		...
		const apply_options_and_sync = (fields) => {
			const options = [""].concat(fields || []).join("\n");
			const source_df = frappe.meta.get_docfield(cdt, "source_field", cdn);

			if (source_df) {
				source_df.options = options;
			}

			if (row.source_field && !(fields || []).includes(row.source_field)) {
				row.source_field = "";
				row.field_datatype = "";
				row.is_attachment = 0;
			}

			frappe.notification.apply_variable_attachment_state(frm, cdt, cdn);
			frm.refresh_field("variables");
		};
	...
	get_email_or_data_fields: function (frm) {
		return frappe.notification.get_reference_fields(frm)
			.filter(df => ["Email", "Data"].includes(df.fieldtype))
			.map(df => df.fieldname);
	},

	get_contact_fields_by_channel: function (frm) {
		return frm.doc.channel === "Email"
			? frappe.notification.get_email_or_data_fields(frm)
			: frappe.notification.get_phone_or_data_fields(frm);
	},

	toggle_channel_fields: function (frm) {
		const isEmail = frm.doc.channel === "Email";

		frm.toggle_display("email_template", isEmail);
		frm.toggle_reqd("email_template", isEmail);

		frm.toggle_display("message_type", !isEmail);
		frm.toggle_display("template", !isEmail && frm.doc.message_type === "Template");
		frm.toggle_display("template_content", !isEmail && frm.doc.message_type === "Template");
		frm.toggle_display("media_url", !isEmail && frm.doc.message_type === "Template");
		frm.toggle_display("create_template", !isEmail && frm.doc.message_type === "Template");
		frm.toggle_display("message_content", !isEmail && frm.doc.message_type !== "Template");
	},
			...
			if (row.recipient_source === "From Profile Field") {
				...
				if (IS_V16_PLUS) {
					frappe.notification.set_child_df_options(
						...
						"clefincode_notification_recipient_list",
						"receiver_field",
						frappe.notification.get_contact_fields_by_channel(frm)
					);
				} else {
					frappe.notification.set_child_df_options(
						cdt,
					...
					if (link_df && link_df.fieldtype === "Link" && link_df.options) {
						frappe.model.with_doctype(link_df.options, function () {
							const linked_fields = (frappe.meta.get_docfields(link_df.options) || [])
								.filter(df => (frm.doc.channel === "Email" ? ["Email", "Data"] : ["Phone", "Data"]).includes(df.fieldtype))
								.map(df => df.fieldname);
...
frappe.ui.form.on("Clefincode Notification", {
	...
	onload: function (frm) {
		...
		frappe.notification.toggle_channel_fields(frm);
	},
	...
	refresh: function (frm) {
		...
		(frm.doc.variables || []).forEach(row => {
				if (row.source_type) {
					...
					frappe.notification.apply_variable_attachment_state(frm, row.doctype, row.name);
				}
			});
