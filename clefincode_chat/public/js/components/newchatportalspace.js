import {
  ...
  const isIndexPage = window.location.pathname === "/homepage" || window.location.pathname === "/" ;
  ...
  export default class ChatPortalSpace {
    ...
    async setup() {
      const userType = frappe.session.user;
      ...
      if (isIndexPage && userType === "Guest") {
        chatboxElement = document.querySelector(".chatbox");
    ...
    async handle_send_message() {
      ...
      if (this.is_first_message == 1 && this.profile.is_verified == 0) {
        ...
        localStorage.setItem("guest_room", results.room);
        localStorage.setItem("guest_respondant", results.respondent_user);
