...
frappe.ui.Notifications = class Notifications {
  ...
  make() {
    this.dropdown = $(".navbar")
      .find(".dropdown-notifications")
      .removeClass("hidden");
...
class NotificationsView extends BaseNotificationsView {
  ...
  get_dropdown_item_html(field) {
    ...
    const is_chat_topic_notification =
      field.chat_topic == 1 && field.approved == 0 ? true : false;
    let message_html = `<div class="message">
			<div>${message}</div>
			<div class="notification-timestamp text-muted"  ${
        is_chat_topic_notification
          ...
          : ``
				...
				${
          is_chat_topic_notification
            ? `<div class='btn btn-primary approved-btn' data-chat-topic = '${field.email_content}' data-reciever = '${field.from_user}' data-doctype = '${field.document_type}' data-docname = '${field.document_name}'>Approved</div>`
            : ``
        }
			</div>			
		</div>`;
  ...
  setup_notification_listeners() {
    ...
    this.parent.on("show.bs.dropdown", () => {
      ...
      if (
        this.notifications_icon.find(".notifications-unseen").is(":visible")
      ) {
        this.toggle_notification_icon(true);
        frappe.call(
