...
def send_notification_via_firebase(registration_token, info, realtime_type, platform=None, title=None, body=None, same_user=None, is_call=False):
    firebase_server_key = frappe.db.get_single_value("ClefinCode Chat Settings", "firebase_server_key")
    if not firebase_server_key:
        return
    # registration_token="af41623e9201776b090e179150cafe98d633485b8c4eac06750d1cced3a3761e"
    ...
    if realtime_type in ["typing", "update_sub_channel_for_last_message"] or same_user == 1:
        ...
        if platform == "ios":
            try:
                ...
                # Add VoIP fields if `is_call` is True
                if is_call:
                    data_message.update({
                        "id": info["id"],
                        "nameCaller": info["nameCaller"],
                        "handle": info["handle"],
                        "isVideo": "false",
                        "aps": {
                            "alert":  "Incoming Call"
            ...
            try:
                ...
                push_service.notify_single_device(
                    ...
                    data_message={
                        ...
                        "same_user": same_user,
                        "id": info.get("id", ""),
                        "nameCaller": info.get("nameCaller", ""),
                        "handle": info.get("handle", ""),
                        "isVideo": info.get("isVideo", "false"),
                        "aps": {
                            "alert": title or "Incoming Call"
                        }
                    },
                    content_available=True
                )
